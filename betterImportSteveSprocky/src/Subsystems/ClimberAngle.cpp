...
ClimberAngle::ClimberAngle() : Subsystem("ClimberAngle") {
	...
	string = 1; //filler number. need to change
	constant = 1; //filler number. need to change
...
int ClimberAngle::GetAngle(){
	...
	double angle = acos (((arm * arm + constant * constant - string * string)/(2 * arm * constant)) * (180.0 / PI));
	//returns the angle of the potentiometer.
	return analogPotentiometer1->Get(angle);
}
